...
func checkPolecatSafety(target polecatTarget) *SafetyCheckResult {
	...
	if infoErr != nil || polecatInfo == nil {
		result.Reasons = append(result.Reasons, fmt.Sprintf("polecat_status=unknown: %v", infoErr))
		result.Blocked = true
		return result
	}
	...
	status := RecoveryStatus{
		Rig:     target.rigName,
		Polecat: target.polecatName,
		Branch:  polecatInfo.Branch,
		Issue:   polecatInfo.Issue,
	}
	fields, err := populateRecoveryStatus(&status, bd, agentBeadID, polecatInfo)
	if err != nil {
		result.Reasons = append(result.Reasons, fmt.Sprintf("recovery_status=unknown: %v", err))
		result.Blocked = true
		return result
	}

	result.CleanupStatus = status.CleanupStatus
	result.ActiveMR = status.ActiveMR
	result.OpenMR = status.ActiveMR
	if fields != nil {
		result.HookBead = agentHookBead(nil, fields)
		if result.HookBead != "" {
			if _, terminal, blocker := hookBeadSafeForCleanup(bd, result.HookBead); terminal {
	...
	targetRefs, _ := recoveryTargetRefs(bd, status.Issue, status.ActiveMR, status.Branch)
	if gitState, gitErr := getGitStateWithTargets(polecatInfo.ClonePath, targetRefs); gitErr == nil {
		result.GitState = gitState
	}

	applyRecoveryStatusToSafetyResult(result, &status)
	return result
}

func applyRecoveryStatusToSafetyResult(result *SafetyCheckResult, status *RecoveryStatus) {
	if result == nil || status == nil {
		return
	}
	if !status.SafeToNuke {
		result.Reasons = append(result.Reasons, status.Blockers...)
		if len(result.Reasons) == 0 {
			reason := status.Reason
			if reason == "" {
				reason = status.Verdict
			}
